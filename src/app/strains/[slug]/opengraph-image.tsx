import { ImageResponse } from "next/og";
import { getStrainBySlug } from "@/lib/db";

export const size = { width: 1200, height: 630 };
export const contentType = "image/png";

export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const strain = await getStrainBySlug(slug);

  const typeColor: Record<string, string> = {
    indica: "#a855f7",
    sativa: "#22c55e",
    hybrid: "#3b82f6",
  };
  const accent = strain ? (typeColor[strain.type] ?? "#4ade80") : "#4ade80";

  return new ImageResponse(
    (
      <div
        style={{
          width: "100%",
          height: "100%",
          display: "flex",
          flexDirection: "column",
          background: "#0a150a",
          fontFamily: "sans-serif",
          position: "relative",
        }}
      >
        {/* Accent bar */}
        <div style={{ position: "absolute", top: 0, left: 0, width: 8, height: "100%", background: accent }} />

        {/* Content */}
        <div style={{ display: "flex", flexDirection: "column", flex: 1, padding: "60px 72px" }}>
          {/* Header */}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 48 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 36, height: 36, borderRadius: 8, background: "#4ade80", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span style={{ color: "#0a150a", fontSize: 20, fontWeight: 700 }}>G</span>
              </div>
              <span style={{ color: "#4ade80", fontSize: 20, fontWeight: 600 }}>GrowingWeed.com</span>
            </div>
            {strain && (
              <span style={{ background: accent + "33", border: `1px solid ${accent}55`, color: accent, borderRadius: 999, padding: "6px 16px", fontSize: 16, textTransform: "capitalize" }}>
                {strain.type}
              </span>
            )}
          </div>

          {/* Title */}
          <div style={{ display: "flex", flexDirection: "column", flex: 1, justifyContent: "center" }}>
            <div style={{ color: "#6b7280", fontSize: 20, marginBottom: 16, textTransform: "uppercase", letterSpacing: 2 }}>Cannabis Strain</div>
            <div style={{ color: "#f9fafb", fontSize: 72, fontWeight: 800, lineHeight: 1.1, marginBottom: 24 }}>
              {strain?.name ?? slug.replace(/-/g, " ")}
            </div>
            {strain && (
              <div style={{ display: "flex", gap: 32, color: "#9ca3af", fontSize: 22 }}>
                <span>THC {strain.thc_min}–{strain.thc_max}%</span>
                <span>·</span>
                <span>CBD {strain.cbd_min}–{strain.cbd_max}%</span>
                {strain.rating > 0 && (
                  <>
                    <span>·</span>
                    <span>★ {strain.rating.toFixed(1)}</span>
                  </>
                )}
              </div>
            )}
          </div>
        </div>
      </div>
    ),
    size
  );
}
