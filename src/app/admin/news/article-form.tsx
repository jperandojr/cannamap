...
const CATEGORIES = ["News", "Legalization", "Science", "Business", "Culture", "Health", "Growing"];

export function ArticleForm({ action, initialData }: Props) {
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        ...
        <div>
          <label className="block text-sm font-medium text-[var(--foreground)] mb-1.5">Author Avatar URL</label>
          <input
            name="author_avatar"
            type="url"
            defaultValue={d?.author_avatar}
            className="w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm text-[var(--foreground)] placeholder:text-[var(--muted)] focus:outline-none focus:ring-2 focus:ring-[var(--primary)]/20 focus:border-[var(--primary)]"
            placeholder="https://..."
          />
        </div>
      ...
      <div className="flex items-center justify-end gap-3 pt-2 border-t border-[var(--border)]">
        ...
        >
          {isPending ? "Saving…" : d ? "Update Article" : "Create Article"}
        </button>
      </div>
    </form>
  );
