...
import { getDispensaries, PAGE_SIZE } from "@/lib/db";
...
interface Props {
  searchParams: Promise<{ q?: string; state?: string; sort?: string; verified?: string; page?: string }>;
}

export default async function DispensariesPage({ searchParams }: Props) {
  const params = await searchParams;
  const page = Math.max(1, Number(params.page) || 1);
  const { data: dispensaries, count } = await getDispensaries({
    q: params.q,
    state: params.state,
    sort: params.sort,
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1">
          <p className="text-sm text-[var(--muted)] mb-6">
            {count} {count === 1 ? "dispensary" : "dispensaries"} found
          </p>
          ...
          {dispensaries.length === 0 ? (
            ...
            <>
              ...
              <Pagination page={page} totalPages={totalPages} basePath="/dispensaries" searchParams={params} />
            </>
          )}
        </div>
      </div>
